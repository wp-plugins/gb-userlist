...
class GB_UserList {
	...
	var $version = '0.3'; //Changer pour correspondre à la version courante
...
//---------------------------------------------
	...
	function AdminHelpPage() {
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					<th scope="row"><?php _e('Display user name?', $this->domain); ?>: </th>

					<td><label><input type="checkbox" name="displayname" <?php if ($displayname == 1) echo 'checked'; ?> /><br /><i><?php _e('This option will display user displayname under avatar (if avatar display mode is selected).', $this->domain); ?></i></label></td>
				</tr>
				<tr>
					<th scope="row"></th>
	...
	function action_wp_print_styles() {
		if(@file_exists(TEMPLATEPATH.'/gb-userlist.css')) {
			wp_enqueue_style('gb-userlist', get_stylesheet_directory_uri().'/gb-userlist.css', false, '0.3', 'all');
		} else {
			wp_enqueue_style('gb-userlist', plugins_url('gb-userlist/gb-userlist.css'), false, '0.3', 'all');
		}	
	}
	...
	function GB_UserList_Widget($args) {
		...
		if ($userlist) {
			...
			foreach ($userlist as $user) {
				...
				if ($style == 0) {
					$output .= '<li class="' . $css_style . '"><a href="/wp-admin/profile.php?user_id=' . $user->ID . '">' . get_avatar($user->ID, $size = $avatarsize);
					if ($displayname == 1) :
